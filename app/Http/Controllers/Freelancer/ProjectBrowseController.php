...

{
    public function index(Request $request): View
    {
        ...
        // Dukung dua nama parameter:
        // 'category' untuk halaman /proyek
        // 'category_id' untuk halaman /projects
        ...
        $sortOptions = [
            ...
            'budget-tinggi' => fn ($q) =>
                $q->orderByRaw(
                    'budget IS NULL, budget DESC'
                ),

            'budget-rendah' => fn ($q) =>
                $q->orderByRaw(
                    'budget IS NULL, budget ASC'
                ),
        ];
    ...
    public function create(Project $project)
    {
        ...
        // proyek harus masih menerima penawaran
        if (!$project->acceptsOffers()) {
            return redirect()
                ...
                ->with(
                    'error',
                    'Proyek ini sudah tidak menerima penawaran baru.'
                );
        }

        // Cek apakah freelancer sudah pernah
        // mengirim penawaran
        $alreadyOffered = Penawaran::where(
            'project_id',
            $project->id
        )
            ->where(
        ...
        if ($alreadyOffered) {
            return redirect()
                ...
                ->with(
                    'error',
                    'Anda sudah pernah mengirim penawaran pada proyek ini.'
                );
        }
    ...
    public function store(
        ...
        // Backend source of truth:
        // tolak jika proyek tidak menerima penawaran
        if (!$project->acceptsOffers()) {
            return redirect()
                ->route(
                    'freelancer.projects.show',
        ...
        if (!$completionService->isComplete(Auth::user())) {
            return redirect()
                ...
                ->with(
                    'error',
                    'Profil Anda belum lengkap. Silakan lengkapi minimal 80% profil terlebih dahulu agar dapat mengirim penawaran.'
                );
        }
        ...
        // Buat notifikasi untuk pemilik proyek
        if (
            $project->owner &&
            $project->owner->id !== Auth::id()
        ) {
            NotificationService::sendTo(
                ...
                message:
                    Auth::user()->name .
                    ' mengirimkan penawaran untuk proyek "' .
