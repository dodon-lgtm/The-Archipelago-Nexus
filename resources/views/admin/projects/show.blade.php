...
@section('content')
    <div class="mb-6">
        <a href="{{ route('admin.projects.index') }}" class="text-sm text-cyan-600 hover:text-cyan-700 font-semibold mb-2 inline-block">
            <i class="fa-solid fa-arrow-left mr-1"></i> Kembali
        </a>
        <h1 class="text-2xl font-extrabold text-slate-800">Detail Proyek</h1>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4 text-sm">
                    <div>
                        <p class="text-xs text-slate-500 font-semibold">Kategori</p>
                        <p class="font-semibold">{{ $project->category->name ?? '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-slate-500 font-semibold">Budget</p>
                        <p class="font-semibold">{{ $project->budget ? 'Rp ' . number_format($project->budget) : '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-slate-500 font-semibold">Deadline</p>
                        <p class="font-semibold">{{ $project->deadline ?? '—' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-slate-500 font-semibold">Total Penawaran</p>
                        <p class="font-semibold">{{ $totalPenawarans }}</p>
                    </div>
                </div>

                <div class="mb-4">
                    <p class="text-xs text-slate-500 font-semibold mb-1">Skills Dibutuhkan</p>
                    <p class="text-sm">{{ $project->skills ?? '—' }}</p>
                </div>

                <div>
                    <p class="text-xs text-slate-500 font-semibold mb-1">Deskripsi</p>
        ...
        <div class="space-y-6">
                    ...
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="font-semibold px-2 py-0.5 rounded-full text-xs
                                @if($project->workspace->status == 'Selesai') bg-emerald-50 text-emerald-600
                                @elseif($project->workspace->status == 'Menunggu Revisi') bg-amber-50 text-amber-600
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="font-semibold">{{ $project->workspace->freelancer->name ?? '—' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-slate-500">Dibuat</span>
                            <span class="font-semibold">{{ $project->workspace->created_at->format('d M Y') }}</span>
                        </div>
                    </div>
                @else
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                ...
                @if($project->image)
                    <div class="mb-2">
                        <p class="text-xs text-slate-500 font-semibold">Gambar</p>
                        <a href="{{ asset('storage/' . $project->image) }}" target="_blank" class="text-sm text-cyan-600 hover:underline">Lihat Gambar</a>
                    </div>
                @endif
                @if($project->attachment)
                    <div>
                        <p class="text-xs text-slate-500 font-semibold">File</p>
                        <a href="{{ asset('storage/' . $project->attachment) }}" target="_blank" class="text-sm text-cyan-600 hover:underline">Download File</a>
                    </div>
                @endif
            ...
            <div class="bg-white rounded-2xl border border-red-200 shadow-sm p-5">
                ...
                <p class="text-xs text-slate-500 mb-3">Hapus proyek ini secara permanen.</p>
                <form method="POST" action="{{ route('admin.projects.destroy', $project) }}"
                      onsubmit="return confirm('Yakin ingin menghapus proyek ini?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="w-full px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-xl text-sm font-semibold transition">
                        <i class="fa-solid fa-trash-can mr-1"></i> Hapus Proyek
                    </button>
                </form>
            </div>
        </div>
