...
@section('content')
    <div class="mb-6">
        <a href="{{ route('admin.hasil-pekerjaan.index') }}" class="text-sm text-cyan-600 hover:text-cyan-700 font-semibold mb-2 inline-block">
            <i class="fa-solid fa-arrow-left mr-1"></i> Kembali
        </a>
        <h1 class="text-2xl font-extrabold text-slate-800">Detail Hasil Pekerjaan</h1>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
                    ...
                    <div>
                        ...
                        <p class="font-semibold">{{ $workspace->project->category->name ?? '—' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div class="px-5 py-4 border-b border-slate-100">
                    <h2 class="font-bold text-slate-800">Riwayat Progress</h2>
                </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div class="px-5 py-4 border-b border-slate-100">
                    <h2 class="font-bold text-slate-800">Pesan ({{ $workspace->messages->count() }})</h2>
                </div>
                <div class="divide-y divide-slate-50 max-h-80 overflow-y-auto">
                    @forelse($workspace->messages as $message)
                        <div class="px-5 py-3 hover:bg-slate-50 transition">
                            <div class="flex items-start gap-3">
                                <div class="w-7 h-7 rounded-full bg-slate-100 text-slate-600 flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if($message->type == 'system')
                                            <span class="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-500 font-semibold">System</span>
                                        @endif
        ...
        <div class="space-y-6">
                ...
                @if($workspace->latestProgress)
                    <div class="text-center mb-3">
                        <div class="text-4xl font-extrabold
                            @if($workspace->latestProgress->progress >= 100) text-emerald-500
                            @elseif($workspace->latestProgress->progress >= 50) text-cyan-500
                            @else text-amber-500 @endif">
                            {{ $workspace->latestProgress->progress }}%
                        </div>
                        <p class="text-xs text-slate-500 mt-1">Tahap: {{ $workspace->latestProgress->stage }}</p>
                    </div>
                    <div class="w-full bg-slate-100 rounded-full h-3 overflow-hidden">
                        <div class="h-full rounded-full transition-all duration-500
                            @if($workspace->latestProgress->progress >= 100) bg-emerald-500
                            @elseif($workspace->latestProgress->progress >= 50) bg-cyan-500
                            @else bg-amber-500 @endif"
                            style="width: {{ $workspace->latestProgress->progress }}%">
                        </div>
                    </div>
                @else
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="font-semibold">{{ $workspace->project->budget ? 'Rp ' . number_format($workspace->project->budget) : '—' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-slate-500">Deadline</span>
                        <span class="font-semibold">{{ $workspace->project->deadline ?? '—' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-slate-500">Dibuat</span>
                        <span class="font-semibold">{{ $workspace->project->created_at->format('d M Y') }}</span>
                    </div>
                </div>
                <a href="{{ route('admin.projects.show', $workspace->project) }}" class="mt-3 inline-block text-xs text-cyan-600 hover:text-cyan-700 font-semibold">
                    Lihat Detail Proyek →
                </a>
            </div>
            ...
            {{-- Timeline --}}
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                <h3 class="font-bold text-slate-800 mb-3">Timeline</h3>
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        <span class="text-slate-500">Mulai</span>
                        <span class="font-semibold">{{ $workspace->created_at->format('d M Y') }}</span>
                    </div>
                    @if($workspace->status == 'Selesai')
                        <div class="flex justify-between">
                            <span class="text-slate-500">Selesai</span>
                            <span class="font-semibold text-emerald-600">{{ $workspace->updated_at->format('d M Y') }}</span>
                        </div>
                    @endif
                </div>
            </div>
