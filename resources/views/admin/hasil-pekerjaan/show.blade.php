...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-blue-100 shadow-sm overflow-hidden">
                <div class="px-5 py-4 border-b border-blue-50"><h2 class="font-bold text-slate-800">Pesan ({{ $workspace->messages->count() }})</h2></div>
                <div class="divide-y divide-slate-50 max-h-80 overflow-y-auto">
                    @forelse($workspace->messages as $message)
                        <div class="px-5 py-3 hover:bg-[#f6f9ff] transition">
                            <div class="flex items-start gap-3">
                                <div class="w-7 h-7 rounded-full bg-blue-50 text-slate-600 flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">{{ strtoupper(substr($message->sender->name ?? '?', 0, 1)) }}</div>
