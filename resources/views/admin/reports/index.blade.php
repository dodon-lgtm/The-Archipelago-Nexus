...
@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-extrabold text-slate-800">Kelola Laporan</h1>
        <p class="text-sm text-slate-500 mt-1">Semua laporan dari pengguna</p>
    </div>
        ...
        <form method="GET" action="{{ route('admin.reports.index') }}" class="flex flex-wrap gap-3 items-end">
            <div class="flex-1 min-w-[200px]">
                ...
                <input type="text" name="search" value="{{ request('search') }}"
                       ...
                       placeholder="Cari pelapor, deskripsi, atau kategori...">
            </div>
            <div class="w-40">
            ...
            <a href="{{ route('admin.reports.index') }}" class="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-sm font-semibold transition">
                Reset
            </a>
        </form>
    </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-slate-50 border-b border-slate-200">
                    <tr>
                        ...
                        <th class="text-left px-5 py-3 font-bold text-slate-600 text-xs uppercase">Deskripsi</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($reports as $report)
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <td class="px-5 py-4">
                                <span class="text-xs px-2 py-1 rounded-full bg-slate-100 text-slate-600 font-semibold">
                                    {{ $report->category }}
                                </span>
                            </td>
                            <td class="px-5 py-4 text-slate-600">{{ $report->project->project_name ?? '—' }}</td>
                            <td class="px-5 py-4 max-w-[200px]">
                                <p class="truncate text-slate-600">{{ $report->description }}</p>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-5 py-12 text-center text-sm text-slate-400">Belum ada laporan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
