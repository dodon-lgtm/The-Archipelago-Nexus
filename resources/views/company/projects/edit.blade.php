        ...
        <main class="flex-1 overflow-y-auto p-6">
            <div class="max-w-4xl mx-auto space-y-6">
                ...
                <nav class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-blue-300 dark:text-slate-400">
                    ...
                    <i class="fa-solid fa-chevron-right text-[9px] text-blue-200"></i>
                    <a href="{{ route('company.projects.show', $project) }}" class="hover:text-blue-600 transition-colors dark:hover:text-blue-400">Detail</a>
                    <i class="fa-solid fa-chevron-right text-[9px] text-blue-200"></i>
                    <span class="text-blue-600 dark:text-blue-400">Edit Proyek</span>
                </nav>
                ...
                @if(!empty($lock['note']) && count($lock['locked']) > 1)
                    <div class="overflow-hidden relative bg-amber-50 border border-amber-200 p-4 rounded-2xl flex items-start gap-3 shadow-sm dark:bg-amber-900/40 dark:border-amber-900">
                        <div class="w-8 h-8 rounded-full bg-amber-100 text-amber-600 border border-amber-200 flex items-center justify-center shrink-0 dark:bg-amber-900/40 dark:text-amber-300 dark:border-amber-900">
                ...
                @endif
                @if(session('error'))
                    <div class="overflow-hidden relative bg-rose-50 border border-rose-200 p-4 rounded-2xl flex items-center gap-4 shadow-sm dark:bg-rose-900/40 dark:border-rose-900">
                        <div class="w-8 h-8 rounded-full bg-rose-100 text-rose-600 border border-rose-200 flex items-center justify-center shrink-0 dark:bg-rose-900/40 dark:text-rose-300 dark:border-rose-900">
                            <i class="fa-solid fa-circle-exclamation text-sm"></i>
                        </div>
                        <div class="font-bold text-rose-800 text-sm dark:text-rose-300">{{ session('error') }}</div>
                    </div>
                @endif
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select name="category_id" {{ $isLocked('category_id') ? 'disabled' : '' }}
                                        class="w-full px-5 py-3.5 bg-blue-50/50 border @error('category_id') border-blue-500 ring-2 ring-blue-500/20 @else border-blue-100 @enderror rounded-xl text-sm font-bold text-blue-950 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 focus:bg-white transition-all cursor-pointer dark:bg-slate-800 dark:border-slate-700 dark:text-white dark:focus:bg-slate-800 {{ $isLocked('category_id') ? 'opacity-60 cursor-not-allowed' : '' }}">
                                ...
                                {{-- Deskripsi --}}
                                <div class="lg:col-span-2">
                                    <label class="block text-[10px] font-black text-blue-500 uppercase tracking-widest mb-2 dark:text-blue-400">Deskripsi Proyek</label>
                                    <textarea name="project_description" rows="6" {{ $isLocked('project_description') ? 'disabled' : '' }}
                                        class="w-full px-5 py-4 bg-blue-50/50 border @error('project_description') border-blue-500 ring-2 ring-blue-500/20 @else border-blue-100 @enderror rounded-xl text-sm font-medium text-blue-950 leading-relaxed focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 focus:bg-white transition-all placeholder:text-blue-300 resize-y dark:bg-slate-800 dark:border-slate-700 dark:text-white dark:focus:bg-slate-800 dark:placeholder:text-slate-500 {{ $isLocked('project_description') ? 'opacity-60 cursor-not-allowed' : '' }}"
                        ...
                        <div class="mb-8">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    @if($project->image)
                                        <div class="mb-2 rounded-xl overflow-hidden border border-blue-200 dark:border-slate-700">
                                    ...
                                    <div class="relative w-full border-2 border-dashed @error('image') border-blue-400 @else border-blue-200 @enderror rounded-xl bg-blue-50/30 hover:bg-blue-50/80 hover:border-blue-400 transition-colors duration-300 dark:bg-slate-800/30 dark:hover:bg-slate-800/80 dark:hover:border-slate-700">
                                        <input type="file" name="image" accept="image/*" {{ $isLocked('image') ? 'disabled' : '' }}
                                            class="w-full px-5 py-4 text-sm text-blue-900 cursor-pointer dark:text-white file:cursor-pointer file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-[10px] file:font-black file:uppercase file:tracking-wider file:bg-blue-600 file:text-white hover:file:bg-blue-700 file:transition-colors focus:outline-none {{ $isLocked('image') ? 'opacity-50 cursor-not-allowed' : '' }}">
                                    </div>
                                ...
                                <div>
                                    ...
                                    @if($project->attachment)
                                        <a href="{{ asset('storage/' . $project->attachment) }}" target="_blank" class="mb-2 inline-flex items-center gap-2 text-[10px] font-bold text-blue-600 bg-blue-50 border border-blue-200 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition dark:text-blue-400 dark:bg-slate-800 dark:border-slate-700 dark:hover:bg-slate-700">
                                    ...
                                    <div class="relative w-full border-2 border-dashed @error('attachment') border-blue-400 @else border-blue-200 @enderror rounded-xl bg-blue-50/30 hover:bg-blue-50/80 hover:border-blue-400 transition-colors duration-300 dark:bg-slate-800/30 dark:hover:bg-slate-800/80 dark:hover:border-slate-700">
                                        <input type="file" name="attachment" accept=".pdf,.doc,.docx" {{ $isLocked('attachment') ? 'disabled' : '' }}
                                            class="w-full px-5 py-4 text-sm text-blue-900 cursor-pointer dark:text-white file:cursor-pointer file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-[10px] file:font-black file:uppercase file:tracking-wider file:bg-blue-600 file:text-white hover:file:bg-blue-700 file:transition-colors focus:outline-none {{ $isLocked('attachment') ? 'opacity-50 cursor-not-allowed' : '' }}">
                                    </div>
