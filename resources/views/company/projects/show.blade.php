...
<style>
    ...
    .animate-pop {
        animation: scalePop 0.3s ease both;
    }
    ...
    .reveal-fallback {
        animation: forceShow 0.01s 0.8s forwards;
    }
    ...
    .custom-modal.active .modal-box {
        animation: modalBoxIn 0.25s ease forwards;
    }
    ...
    .custom-modal.active .modal-icon {
        animation: iconBounce 0.35s ease 0.05s both;
    }
    ...
    .dark .delete-modal-box {
        background: #0f172a;
        border-color: #1e293b;
    }

    .delete-modal.active .delete-modal-box {
        animation: deleteModalIn 0.28s cubic-bezier(.2, .8, .2, 1) forwards;
    }
    ...
    .delete-modal.active .delete-modal-icon {
        animation: deleteIconPop 0.35s ease 0.04s both;
    }
...
<div class="flex-1 min-w-0 flex flex-col min-h-screen">
    ...
    <main class="flex-1 min-w-0 overflow-y-auto">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
            ...
            @if(session('success'))
                <div class="mb-6 flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-950/40 dark:to-teal-950/40 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 rounded-xl text-sm font-medium shadow-sm animate-pop">

                    <div class="w-8 h-8 rounded-full bg-emerald-100 dark:bg-emerald-900/60 flex items-center justify-center shrink-0">
            ...
            @if(session('error'))
                <div class="mb-6 flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-red-50 to-rose-50 dark:from-red-950/40 dark:to-rose-950/40 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 rounded-xl text-sm font-medium shadow-sm animate-pop">

                    <div class="w-8 h-8 rounded-full bg-red-100 dark:bg-red-900/60 flex items-center justify-center shrink-0">
            ...
            {{-- =================================================
                BANNER: NEGOSIASI SELESAI / WORKSPACE AKTIF
            ================================================== --}}

            @if($project->workspace)
                @php $ws = $project->workspace; @endphp

                <div class="mb-6 animate-pop rounded-2xl border border-emerald-200 dark:border-emerald-800 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-950/40 dark:to-emerald-950/30 p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div class="flex items-start gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            <p class="text-sm font-extrabold text-emerald-800 dark:text-emerald-200 flex flex-wrap items-center gap-2">
                                <span>Negosiasi Selesai — Deal Terkunci</span>

                                <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-emerald-500 text-white text-[11px] font-bold">
                                    Selesai / Deal
                                </span>
                            </p>

                            <p class="text-xs leading-relaxed text-emerald-700 dark:text-emerald-300/90 mt-1">
                                Freelancer telah dipilih. Riwayat negosiasi tetap tersimpan (read-only). Komunikasi pengerjaan dilanjutkan di Workspace.
                            </p>

                            @php
                                $winner = $project->penawarans->firstWhere('status', 'Diterima');
                            @endphp

                            @if($winner)
                                <p class="text-[11px] text-slate-600 dark:text-slate-400 mt-2">
                                    <span class="font-semibold">Freelancer terpilih:</span>
                                    {{ $winner->freelancer->name ?? '-' }}
                                    ·
                                    <span class="font-semibold">Harga deal:</span>
                                    Rp {{ number_format($winner->harga_penawaran, 0, ',', '.') }}
                                    ·
                                    <span class="font-semibold">Estimasi:</span>
                                    {{ $winner->estimasi_hari }} hari
                                </p>
                            @endif
                        </div>
                    </div>
                    ...
                    <a href="{{ route('company.workspaces.show', $ws) }}"
                       class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold transition-colors shrink-0 shadow-sm">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        ...
                        <div class="p-3.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-xl flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-[11px] font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                    Budget Proyek
                                </p>
                        ...
                        <div class="text-xs sm:text-sm text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/30 p-4 rounded-xl border border-slate-100 dark:border-slate-800 leading-relaxed">
                            ...
                            @if(strlen($project->project_description ?? '') > 200)
                                <button @click="expanded = !expanded"
                                        class="text-xs font-semibold text-blue-600 dark:text-blue-400 hover:underline mt-2 inline-block">
            ...
            @php
                ...
                $penawaranData = $project->penawarans->map(function($p) {
                    ...
                    $photoUrl = $foto
                        ? (Str::startsWith($foto, ['http://', 'https://'])
                            ? $foto
                            : asset('storage/' . $foto))
                        : null;
            ...
            >
                ...
                <div class="p-6">
                    ...
                    @if($project->penawarans->isEmpty())
                        ...
                        {{-- EMPTY STATE --}}
                        <div class="relative overflow-hidden py-16 text-center rounded-2xl bg-gradient-to-br from-blue-50 via-sky-50 to-indigo-50 dark:from-slate-800/60 dark:via-slate-800/40 dark:to-slate-800/20">

                            <div class="w-16 h-16 mx-auto mb-5 bg-white dark:bg-slate-800 shadow-sm rounded-2xl flex items-center justify-center">
                        ...
                        <template x-if="filteredItems.length === 0">
                            <div class="py-12 text-center">
                                <p class="text-xs font-semibold text-slate-400">
                                    Tidak ada penawaran yang cocok dengan filter atau pencarian Anda.
                                </p>
                            </div>
                        </template>
                        ...
                        <div class="space-y-4">
                            ...
                            @foreach($project->penawarans as $penawaran)
                                ...
                                @php
                                    ...
                                    $isNegoLocked = $penawaran->isNegotiationLocked();
                                @endphp
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        @php
                                            ...
                                            $profileUrl = $penawaran->freelancer
                                                ? route('company.freelancer.profile', $penawaran->freelancer->id)
                                                : '#';
                                        @endphp

                                        <a href="{{ $profileUrl }}"
                                           @click.stop
                                           class="flex items-center gap-3 min-w-0 mr-auto hover:opacity-80 transition-opacity">
                                        ...
                                        @if($isNegoLocked)
                                            ...
                                            <span class="hidden sm:inline-flex items-center gap-1 px-2 py-1 rounded-full {{ $penawaran->status === 'Diterima' ? 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800/50' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700' }} border text-[10px] font-bold">
                                                ...
                                                <i class="fa-solid {{ $penawaran->status === 'Diterima' ? 'fa-handshake' : 'fa-lock' }} text-[10px]"></i>

                                                {{ $penawaran->status === 'Diterima' ? 'Deal' : 'Dikunci' }}
                                        ...
                                        {{-- CHEVRON --}}
                                        <i class="fa-solid fa-chevron-down text-slate-400 text-xs shrink-0 transition-transform"
                                    ...
                                    <div x-show="open"
                                         x-transition
                                         class="px-5 pb-5 pt-1 border-t border-slate-100 dark:border-slate-800">
                                        ...
                                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                                            ...
                                            <div>
                                                ...
                                                @if($penawaran->proposal)
                                                    ...
                                                    <a href="{{ asset('storage/' . $penawaran->proposal) }}"
                                                       target="_blank"
                                            ...
                                            @if($isNegoLocked)
                                                ...
                                                <div class="w-full mt-3 rounded-xl border {{ $penawaran->status === 'Diterima' ? 'border-emerald-200 dark:border-emerald-800/50 bg-emerald-50/60 dark:bg-emerald-950/20' : 'border-amber-200 dark:border-amber-800/40 bg-amber-50/60 dark:bg-amber-950/20' }} p-3 flex flex-col gap-2">

                                                    <p class="text-[11px] font-extrabold {{ $penawaran->status === 'Diterima' ? 'text-emerald-700 dark:text-emerald-300' : 'text-amber-700 dark:text-amber-300' }} flex items-center gap-1.5">

                                                        <i class="fa-solid {{ $penawaran->status === 'Diterima' ? 'fa-circle-check' : 'fa-lock' }}"></i>

                                                        {{ $penawaran->status === 'Diterima' ? 'Deal — Harga Disepakati' : 'Negosiasi Dikunci' }}

                                                        <span class="ml-1 inline-flex items-center px-2 py-0.5 rounded-full bg-white dark:bg-slate-900 border text-[10px] font-bold {{ $penawaran->status === 'Diterima' ? 'text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800' : 'text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700' }}">
                                                            {{ $penawaran->status }}
                                                        </span>
                                                    ...
                                                    <div class="grid grid-cols-2 gap-2 text-[11px]">
                                                        ...
                                                        <div class="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-2">
                                                            ...
                                                            <p class="text-slate-500 dark:text-slate-400 text-[10px] uppercase tracking-wide font-semibold">
                                                                Harga Deal
                                                            </p>
                                                    ...
                                                    <p class="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                                                        Riwayat negosiasi tetap tersimpan di bawah. Klik
                                                        <span class="font-bold">Lihat Riwayat Negosiasi</span>
                                                        untuk membaca percakapan (read-only).
                                                        Komunikasi pengerjaan dilanjutkan di Workspace.
                                                    </p>
                                            ...
                                            <div class="flex flex-wrap items-center gap-2">
                                                ...
                                                <button type="button"
                                                        ...
                                                        data-peer-name="{{ $penawaran->freelancer->name ?? 'Freelancer' }}"
                                                        data-peer-type="freelancer"
                                                        class="inline-flex items-center gap-2 px-4 py-2 border {{ $isNegoLocked ? 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700' : 'border-blue-200 dark:border-blue-900/50 text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40 hover:bg-blue-100 dark:hover:bg-blue-900/60' }} rounded-lg text-xs font-bold transition-colors">

                                                    <i class="fa-{{ $isNegoLocked ? 'solid fa-clock-rotate-left' : 'regular fa-comments' }}"></i>

                                                    {{ $isNegoLocked ? 'Lihat Riwayat Negosiasi' : 'Negosiasi' }}

                                                    @if(!$isNegoLocked && ($negoUnread[$penawaran->id] ?? 0) > 0)
                                                    ...
                                                    @elseif($isNegoLocked)
                                                        ...
                                                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-[10px] font-bold">
                                                ...
                                                @if($isNegoLocked && $project->workspace)

                                                    <a href="{{ route('company.workspaces.show', $project->workspace) }}"
                                                       class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold transition-colors shadow-sm">
...
<div id="deleteProjectModal"
    ...
    <div class="delete-modal-box"
        ...
        <div class="delete-modal-warning">
            ...
            <span>
                Proyek yang sudah dihapus tidak dapat dipulihkan.
                Pastikan kamu benar-benar ingin melanjutkan.
            </span>
...
<script>
    ...
    let selectedDeleteProjectForm = null;

    function openDeleteProjectModal(button) {

        selectedDeleteProjectForm =
            button.closest(
                '.delete-project-form'
            );
    ...
    document.addEventListener(
        ...
        function (e) {
            ...
            if (
                selectModal &&
                selectModal.classList.contains(
                    'active'
                )
            ) {
